...
3. Sediakan tetapan fail `server.config.js` untuk _run_ di pc anda. Tetapan ini penting untuk mengelakkan web ni run pada port yang sama.

	```
	cp config.sample.js server.config.js
	```

	Kemudian sunting `server.config.js` ikut kesesuaian sendiri.
